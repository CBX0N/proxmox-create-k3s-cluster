locals {
  ...
  scp_command = [ for node, conf in var.nodes : "scp ${var.cluster_config.admin_user}@${var.proxmox_vm_config.ip_prefix}.${conf.vmid}:/etc/rancher/k3s/k3s.yaml ${path.module}/k3s.yaml" ]
